    ...
    if (result.isConfirmed) {
      ExperienceService.delete(id)
        ...
        .catch((err) => {
          console.log(err);
          toast.error(
            "An error occured while trying to deleting this work experience. Please try again"
          );
        })
        .finally(() => {
...
async function getuserExperience() {
  ...
  ExperienceService.all()
    ...
    .catch((err) => {
      console.log(err);
      toast.error(
        "Could not fetch your work experience details at the moment please referesh page"
      );
    })
    .finally(() => {
      fetching.value = false;
