<template>
  <main class="flex-1 pb-8">
    <div v-if="loading" class="space-y-4">
      <detail-skeleton class="px-5 py-4 border rounded-lg shadow-lg" :class="'bg-white border-slate-200'"
        v-for="n in 10" :key="n"></detail-skeleton>
    </div>

    <div v-if="!loading" class="px-2 py-8 mx-auto max-w-7xl">
      <div
        class="max-w-3xl px-4 mx-auto mt-12 mb-8 md:flex md:items-center md:justify-between md:space-x-5 lg:max-w-7xl">
        <div class="flex items-center space-x-5">
          <div class="flex-shrink-0">
            <div class="relative">
              <img class="w-16 h-16 rounded-full" src="../../assets/images/icon.png" alt="" />
          ...
          <div>
            <h1 class="mt-2 text-2xl font-bold leading-7 text-gray-900 title-case sm:text-3xl sm:truncate">
              ...
              <small class="px-4 py-1 ml-5 text-sm bg-indigo-200 rounded-full">
                ...
                Closing
                <timeago class="ml-2" v-if="job.deadline" :datetime="job.deadline" />
              </small>
            </h1>
            ...
            <div class="flex flex-col mt-1 sm:flex-row sm:flex-wrap sm:mt-0 sm:space-x-8">
              ...
              <div v-if="job.city && job.city.region && job.city.region.country"
              ...
              <template v-if="job.currency">
                <div class="flex items-center mt-2 text-sm text-gray-500">
                  ...
                  {{ Number(job.salary_min).toLocaleString() }} &ndash;
        ...
        <div
          ...
          <router-link to="/account/jobs"
            class="inline-flex items-center justify-center px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-md shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-100 focus:ring-blue-500"><i
              class="mr-2 fa-solid fa-chevron-left"></i>Back</router-link>
          <button type="button" @click="initApply"
            ...
            Apply Now<i class="ml-2 fa-solid fa-paper-plane"></i>
          </button>
        </div>
      </div>
      <div class="w-full px-8 pt-8 pb-12 bg-white border rounded-lg shadow-lg">
        <div class="mt-12">
          <h2 class="mb-2 text-2xl font-bold leading-snug text-slate-800">
            Description
          </h2>
          <div v-html="job.description" class="mb-6 space-y-6"></div>
        </div>
        ...
        <div class="mt-12">
          ...
          <div v-html="job.requirements" class="mb-6 space-y-6"></div>
        </div>
        ...
        <div class="mt-12">
          ...
          <div v-html="job.benefit" class="mb-6 space-y-6"></div>
        </div>
        ...
        <div class="mt-12">
          ...
          <div v-html="job.responsibilities" class="mb-6 space-y-6"></div>
        </div>
            ...
            <DialogPanel
              ...
              <div class="w-100">
                <div class="mt-3 text-center sm:mt-0 sm:text-left">
                  <DialogTitle as="h3" class="px-6 text-lg font-medium leading-6 text-gray-900">
                    Apply For {{ job.title }}
                  </DialogTitle>
                  <div class="mt-2">
                    <div class="py-5 space-y-6 bg-white sm:p-6">
                      <div>
                        <div class="grid grid-cols-6 gap-6">
                          <div v-for="question in questions" :key="question.id" class="col-span-6 sm:col-span-6">
                            <label for="postal-code" class="block mb-2 text-sm font-medium text-gray-700">{{
                              question.question }}</label>
                            <template v-if="
                                question.job_question_type.name ==
                                'Multiple Choice'
                              ">
                              <div class="space-y-4 sm:flex sm:items-center sm:space-y-0 sm:space-x-10">
                                <div v-for="option in question.job_question_options" :key="option.id"
                                  class="flex items-center">
                                  <input v-model="question.response" :value="option.id" type="radio"
                                    class="w-4 h-4 text-indigo-600 border-gray-300 focus:ring-indigo-500" />
                                  <label :for="question.id" class="block ml-3 text-sm font-medium text-gray-700">
                                    {{ option.option }}
                                  </label>
                                </div>
                              </div>
                            ...
                            <template v-else-if="
                              ...
                              <SwitchGroup>
                                <div class="flex items-center">
                                  <Switch v-model="question.response" :class="
                                      question.response
                                        ? 'bg-blue-600'
                                        : 'bg-gray-200'
                                    "
                            ...
                            <template v-else-if="
                                question.job_question_type.name == 'Dropdown'
                              ">
                              <select v-model="question.response"
                                class="block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                                <option disabled value="">Please select</option>
                                <template v-for="option in question.job_question_options" :key="option.id">
                            ...
                            <template v-else-if="
                                question.job_question_type.name == 'Short Text'
                              ">
                              <input type="text" v-model="question.response"
                                class="block w-full mt-1 border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" />
                            </template>
                            <template v-else-if="
                                question.job_question_type.name == 'Long Text'
                              ">
                              <textarea v-model="question.response" rows="5"
                            ...
                            <template v-else-if="
                                question.job_question_type.name == 'Date Time'
                              ">
                              <input type="date" v-model="question.response"
                                class="block w-full mt-1 border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" />
                            </template>
                            <template v-else-if="
                                question.job_question_type.name == 'Number'
                              ">
                              <input type="number" @keypress="numbersOnly" v-model="question.response"
                                class="block w-full mt-1 border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" />
                            </template>
                          </div>
                        </div>
              ...
              <div class="px-6 mt-3 mb-3 sm:mt-4 sm:flex sm:flex-row-reverse">
                <app-button @click="apply" :processing="processing" :disabled="!isFormValid"
                  class="inline-flex justify-center px-4 py-2 ml-3 text-sm font-medium text-white bg-indigo-600 border border-transparent rounded-lg shadow-sm hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">Apply
                </app-button>
                <button type="button"
  ...
  Object.keys(settings).forEach((field) => {
    if (settings[field] === JobSettings.Mandatory && field in userInfo.value) {
      if (userInfo.value[field] === null || userInfo.value[field] === "") {
        requiredFields.push(
          field === "dob" ? "Date of Birth" : CapitalizeFirstLetter(field)
        );
      }
    }
  });

  if (Array.isArray(job.value.job_questions) && job.value.job_questions.length > 0) {
...
function apply() {
  ...
  if (!isInvalid) {
    let payloadQuestions = questions.value.map((question) => {
      return {
        ...
        job_question_option_id:
          question.job_question_type.has_options == true
            ? question.response
            : null,
        response:
          question.job_question_type.has_options == true
    ...
    JobsService.apply(payload)
      .then(() => {
        processing.value = false;
        open.value = false;
        toast.success(
          "Application sent successfully. You will hear back from our team soon."
        );
      })
      .catch((err) => {
        processing.value = false;
...
const isFormValid = computed(() => {
  return questions.value.every(
    (q) => q.response !== null && q.response !== "" && q.response !== undefined
  )
})
